...
def _fast_metric(yt, yhat, yprob, metric):
    """Metric for each row of a stack of bootstrap resamples.

    yt, yhat and yprob are (n_boot, n). Rows containing a single class are
    returned as NaN and dropped by the caller, matching the behaviour of the
    original per-resample loop. Written directly rather than through sklearn
    because the per-call input validation dominates the runtime at these
    sample sizes.
    """
    tp = np.sum(yhat & (yt == 1), axis=1).astype(float)
    fp = np.sum(yhat & (yt == 0), axis=1).astype(float)
    fn = np.sum(~yhat & (yt == 1), axis=1).astype(float)
    single = (yt.sum(axis=1) == 0) | (yt.sum(axis=1) == yt.shape[1])

    if metric == 'precision':
        out = np.divide(tp, tp + fp, out=np.zeros_like(tp), where=(tp + fp) > 0)
    elif metric == 'recall':
        out = np.divide(tp, tp + fn, out=np.zeros_like(tp), where=(tp + fn) > 0)
    elif metric == 'f1':
        den = 2 * tp + fp + fn
        out = np.divide(2 * tp, den, out=np.zeros_like(tp), where=den > 0)
    elif metric in ('pr_auc', 'roc_auc'):
        out = np.empty(yt.shape[0])
        for i in range(yt.shape[0]):
            if single[i]:
                out[i] = np.nan
                continue
            out[i] = (average_precision_score(yt[i], yprob[i])
                      if metric == 'pr_auc' else roc_auc_score(yt[i], yprob[i]))
        return out
    else:
        raise ValueError(f"unsupported metric for the fast path: {metric}")
...
def bootstrap_ci(y_true, y_prob, metric='f1', threshold=0.5, n_boot=2000,
    ...
    y_prob = np.asarray(y_prob)
    n = len(y_true)
    # Draw every resample at once. The loop version spent almost all of its
    # time inside sklearn's per-call validation, which dominates at these
    # sample sizes; the metrics below are cheap to compute directly.
    idx = RNG.integers(0, n, size=(n_boot, n))
    yt = y_true[idx]
    yp = y_prob[idx] >= threshold
    stats = _fast_metric(yt, yp, y_prob[idx], metric)
...
def paired_bootstrap_test(y_true, prob_a, prob_b, metric='f1', threshold=0.5,
    ...
    idx = RNG.integers(0, n, size=(n_boot, n))
    yt = y_true[idx]
    ma = _fast_metric(yt, pa[idx] >= threshold, pa[idx], metric)
    mb = _fast_metric(yt, pb[idx] >= threshold, pb[idx], metric)
    diffs = ma - mb
