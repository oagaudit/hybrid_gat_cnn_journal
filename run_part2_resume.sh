#!/usr/bin/env bash
# ===========================================================================
#  PART 2 — RESUME  (เริ่มจาก STEP 4b)
#
#  ใช้ไฟล์นี้แทน run_part2.sh หลังจากที่รอบก่อนพังตอน XGBoost
#  STEP 0b และ STEP 4a รันผ่านไปแล้ว ไม่ต้องทำซ้ำ
#
#     DEVICE=cpu nohup bash run_part2_resume.sh > part2b.log 2>&1 &
#
#  ถ้า outputs/v2/learned_node_features.pt หายไป ให้รัน run_part2.sh ปกติแทน
# ===========================================================================
set -euo pipefail

# macOS OpenMP guard — นี่คือสาเหตุที่รอบก่อน segfault
...
[ -f "$FEATS" ] || { echo "ไม่พบ $FEATS — STEP 4a ยังไม่เสร็จ ให้รัน run_part2.sh แทน"; exit 1; }

echo "############ PART 2 RESUME เริ่ม $(date) | bridge=$BRIDGE ############"
